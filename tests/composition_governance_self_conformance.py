...
def main() -> int:
    ...
    def k5():
        # content-addressed + order-independent over members (target fixed)
        a = compute_composition_id(TENANT, "lora-stack", ["cert-aaa", "cert-bbb"], "oci://t:1")
        b = compute_composition_id(TENANT, "lora-stack", ["cert-bbb", "cert-aaa"], "oci://t:1")  # reordered
        assert a == b, "composition id not order-independent over members"
