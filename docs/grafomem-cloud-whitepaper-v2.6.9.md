...
> [!TIP]
> The **full conformance suite passes 52/52 in mock mode**. Live runs differ per provider: Anthropic 49/51; Gemini 48/51 — no live run is a clean 51 (see §32). Expanded in Sprint 13 with Phase 22 covering health check endpoints. Every security mechanism is verified both that it blocks AND that it doesn't falsely block. All six production gates (§17.2) are closed.
...
> mode: **52/52**. Live: OpenAI 39-test baseline · Anthropic **49/51** · Gemini **48/51** (see §32).
...
> [!NOTE]
> Sprint 17 adds Anthropic (Claude Opus 4) and Gemini (2.5 Pro) as provider options alongside existing OpenAI and Mock. **What shipped**: adapter plumbing, CLI flags (`--anthropic` / `--gemini`), 16 DB-free unit tests, and live conformance runs for the providers. Anthropic Claude Opus 4 runs at **49/51**; Google Gemini 2.5 Pro at **48/51**. No live run is a clean 51 — non-deterministic LLM output produces expected divergences, and current deployment limits affect a small number of infrastructure integration checks.
...
>
> While core isolation, signing, and erasure tests pass cleanly in the live environment, no provider achieves a clean 51/51 live. Both providers experienced test-environment DB-tamper limitations and infrastructural API gaps (500s/405s) due to the Railway deployment. LLM non-determinism and provider-specific defects also cause expected replay divergences.
...
| **Cloud managed inference** | LIVE | Multi-provider (§32): Anthropic Claude Opus 4 (49/51), Gemini 2.5 Pro (48/51). No live run is a clean 51. |
