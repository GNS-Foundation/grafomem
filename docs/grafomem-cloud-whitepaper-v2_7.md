...
| E2E conformance suite | **~1,784 lines** — Mock **52/52**. Live per-provider (artifact `GRAFOMEM-AUDIT-025 v2.1`, 2026-06-22): Anthropic **36/46**, Gemini **36/46** (7 prod 500 / 2 env / 1 replay; 51→46 denominator excludes offline DB-tamper tests). |
...
| **Conformance validation** | **52/52 platform + 50/50 v3 + 12/12 gcrumbs = 114 gates (mock/local)**. Live (artifact `GRAFOMEM-AUDIT-025 v2.1`, 2026-06-22): Anthropic **36/46**, Gemini **36/46** — 7 prod routing/500, 2 env-blocked offline, 1 replay defect (51→46 = offline DB-tamper tests excluded). |
...
> [!TIP]
> The **full conformance suite passes 52/52 in mock mode**. Live runs differ per provider (artifact `GRAFOMEM-AUDIT-025 v2.1`): OpenAI gpt-4o-mini at the 39-test baseline; Anthropic **36/46**; Gemini **36/46** — no live run is a clean 52; the live misses are 7 prod 500s, 2 env-blocked offline tamper tests, and 1 replay defect (see §32). Expanded in Sprint 13 with Phase 22 covering health check endpoints. Every security mechanism is verified both that it blocks AND that it doesn't falsely block. All six production gates (§17.2) are closed.
...
> mode: **52/52**. Live (artifact `GRAFOMEM-AUDIT-025 v2.1`): OpenAI 39-test baseline · Anthropic **36/46** · Gemini **36/46** (see §32).
...
*End of document. Version 2.7.0 — 26 sprints. Mock conformance 52/52 + 50/50 v3 governed-services + 12/12 gcrumbs = 114 gates (mock/local); GMP self-conformance M8=1.000 (PostgresGMPBackend); HITL resume lifecycle validated two-sided; OpenAPI contract test validates SDK ↔ spec alignment; Prometheus observability with 18 metrics + health probes. Full `pytest tests/` suite 241/241 green (run together). Python SDK 32/33 against live sandbox. Live multi-provider conformance (artifact GRAFOMEM-AUDIT-025 v2.1): OpenAI 39-test baseline; Anthropic 36/46; Gemini 36/46 — open item: 7 production routing/500 errors on the live deployment. pgvector primary (Qdrant adapter built, Phase 6 not yet live); SIEM exporter parked; SLO alerting Defined-not-Active.*
...
>
> Live denominator is 46 (5 offline DB-tamper tests excluded — they need direct DB mutation, blocked against the remote prod network). The 10 live misses break down as **7 production routing/500 errors** (tracked open item — root-cause on the Railway deployment), **2 environment-blocked** offline tamper tests, and **1 provider replay defect** (LLM non-determinism). Only the last is "expected"; the 7 prod 500s are a real open item, not non-determinism.
...
| **Cloud managed inference** | LIVE | Multi-provider (§32): OpenAI `gpt-4o-mini` (39-test baseline), Anthropic Claude Opus 4 (**36/46**), Gemini 2.5 Pro (**36/46**). 7 of the 10 live misses are prod routing/500s (open item). |
