...
>
> **v2.6.7** is a status-reconciliation pass: §9.3 is brought current with Sprint 23b — the scope vocabulary expanded from 14 to **21 scopes** and enforcement from 6 to **all 22 route files** (the `ip_allowlist` schema note corrected from "future" to enforced-at-auth); a canonical sequence note is added to disambiguate the Sprint / Phase / version axes; and the per-sprint test snapshots are flagged as historical pending a current canonical re-count. (Grant text for the seven Sprint-23b scopes is listed per the 23b audit, pending confirmation against `scopes.py`.)
>
> **v2.6.8** reconciles against a **live codebase inventory** (suites executed, `scopes.py` read, route files grepped). Verified corrections: route-file enforcement is **21 files / 203 `require_scope` calls** (not 22); scope grant text updated to authoritative `scopes.py` descriptions; integration-seam suite is **62 tests** (62/62 in isolation); scope tests are **52**. **Three open quality items are now flagged, not hidden:** (1) the unit suite is **225/241 — 7 failing, 9 skipped**, not "112/112 ALL GREEN"; (2) the integration suite passes only run in isolation and **fails under a full `pytest tests/` run** on a missing-`memories`-table fixture issue; (3) live conformance per artifact `GRAFOMEM-AUDIT-025 v2.1` (runs 2026-06-22) is **Mock 52/52, Anthropic 36/46, Gemini 36/46** — materially below the §32 figures (49/51, 48/51) with an unexplained denominator shift. The §32 / Key-Numbers conformance figures are left in place but **marked under re-verification** pending a root-cause note and confirmation of the canonical artifact; they are not yet propagated doc-wide.
>
> **v2.6.9** layers **Sprint 24 (Continuous Assurance activation + Phase 1 DR)** onto the v2.6.8 reconciliation base — the open-items note below is **retained, not reverted**. As-built this sprint: the Continuous Assurance scheduler runs automated drift-detection sweeps wired into the app lifespan (dynamic per-tenant start/stop without restart); data residency adds **logical, region-tagged residency enforced at query time** (single-region store, query-predicate-enforced — *not* physical segregation, and defeatable by an omitted predicate); a vault check showed a direct-DB read returned only ciphertext and a tampered record failed decryption — **demonstrating application-layer envelope encryption on a sample** (tamper-evident, not "impenetrable"); a W6 restore probe **correctly flagged** a resurrected delete against the erasure certificate; DEK `pg_notify` cross-node cache invalidation; SIEM 180-day retention; `region` and `coverage JSONB` schema changes; and three real E2E fixes (Decimal serialization in regulatory reports; the missing `003_protocol_3_4.sql` `coverage` migration applied to prod; chain-tamper/P0-6b tests pointed at the correct backend). Mock E2E is **52/52**. **Two items remain unverified by pasted artifact** (see status note): the claimed full-suite **241/241** has no pasted `pytest tests/` run yet, and the live per-provider conformance gap (artifact 36/46 vs §32's 49/51) is unresolved — both stay flagged, not propagated.

> **v2.7.0** cuts the clean milestone — the v2.6.x reconciliation gates are closed with **pasted artifacts**. Full suite **241/241** green run together (the isolation defect and the stale 112/112 are gone); `test_rest_erasure_fail_closed` verified **fail-closed** (503 + data preserved — fixed, not relaxed); production Postgres + tenant credentials rotated; W6 restore probe run and passing. Live conformance is resolved to the canonical artifact `GRAFOMEM-AUDIT-025 v2.1` (2026-06-22): **Mock 52/52 · Anthropic 36/46 · Gemini 36/46** — the 51→46 denominator excludes 5 offline DB-tamper tests requiring direct DB mutation (blocked against the remote prod network). Of the 10 live misses: **7 are production routing/500 errors, 2 are environment-blocked offline tamper tests, 1 is a provider replay defect** — the 7 prod 500s are the one remaining open item (tracked, not a release gate). Roadmap: pgvector is the primary prod backend (Qdrant adapter built, Phase 6 not yet live); SIEM exporter parked (pending read-replica offload); SLO alerting Defined-not-Active. §32 and Key-Numbers live figures are now propagated and the under-re-verification flags lifted.
...
| E2E conformance suite | **~1,784 lines** — Mock **52/52** (verified on the live Railway deployment, v2.7.1). Live per-provider (last full run 2026-06-22): Anthropic/Gemini **36/46**; the 7 prod-500 contributors are fixed (v2.7.1), so a confirming live re-run is expected ~43/46 (2 env-blocked + 1 replay remain, not product defects). |
...
| **Unit tests** | **241/241 passing** (full `pytest tests/`, run together, 53.71s — pasted artifact). Supersedes the v2.6.1 "112/112"; the 7 prior fixture failures and the full-suite isolation defect are fixed. |
| **Integration seam tests** | **62/62** against real Postgres — RBAC, key rotation, loop detection, timeout, SSE, erasure, **scope enforcement**. Green in isolation AND under the full `pytest tests/` run (isolation defect fixed). |
| **Conformance validation** | **52/52 platform + 50/50 v3 + 12/12 gcrumbs = 114 gates (mock/local)** — mock verified on the live Railway deployment (v2.7.1). Live per-provider (2026-06-22): **36/46**; 7 prod-500 contributors fixed (v2.7.1), confirming live re-run expected ~43/46 (residual 2 env-blocked + 1 replay). |
| **SDK validation** | **32/33 ALL GREEN** (live sandbox server) |
| **GMP self-conformance** | **M8 = 1.000** (7/7 capabilities, PostgresGMPBackend) |

> [!NOTE]
> **v2.7 status — reconciliation gates closed (pasted artifacts).** Full suite **241/241** green run together (53.71s); `test_rest_erasure_fail_closed` verified **fail-closed** (no signing key → 503 AND fact still retrievable — fixed, not relaxed); production Postgres + tenant credentials rotated (Railway), scratch repro scripts purged; W6 restore probe run and passing. Live conformance resolved to artifact `GRAFOMEM-AUDIT-025 v2.1` (2026-06-22): Mock **52/52**, Anthropic **36/46**, Gemini **36/46** (51→46 = 5 offline DB-tamper tests excluded — they need direct DB mutation, blocked against the remote prod network). Route enforcement: 21 files / 203 `require_scope` calls.
>
...
### 3.4 Vector Backend — pgvector (primary) + Qdrant adapter (Phase 6, not yet live)

> [!NOTE]
> As of v2.7, **pgvector on PostgreSQL is the primary production backend** (`PostgresGMPBackend`). The Qdrant adapter below is **built but not the default factory** — it is the Phase-6 migration target (§17.3, R1–R8 unfulfilled), not the live default.

- Embedding model: `BAAI/bge-small-en-v1.5` (384-dim)
- Similarity (Qdrant adapter): cosine distance via Qdrant's exact search
- Schema (Qdrant adapter): collections with mandatory payload indexing on all filterable fields (`tenant_id`, `valid_from`, `valid_until`)
- Tenant isolation: filter-aware query planning. pgvector is the supported, live default; Qdrant is the planned scale-out option.
...
| `compliance:read` | Compliance & regulatory — view reports/dashboards |
| `compliance:admin` | Compliance & regulatory — manage configuration |
...
| `artifacts:read` | Governed artifacts — read |
| `artifacts:admin` | Governed artifacts — manage/generate |
| `manifold:read` | Manifold & templates — view (visual governance) |
| `llm:admin` | Manage LLM provider configurations |
| `webhooks:admin` | Manage webhook endpoints |
| `sso:admin` | SSO / SAML configuration |
| `keys:admin` | Manage API keys |
| `admin:platform` | Platform admin (tenant CRUD, billing) |
| `*` | Superuser — all scopes |

> [!NOTE]
> **21 scopes as of Sprint 23b.** Sprint 23 shipped the first 14; Sprint 23b added the seven `compliance:*`, `artifacts:*`, `manifold:read`, `sso:admin`, and `admin:platform` scopes and expanded enforcement to **21 route files** (§9.3.3; the 23b changelog said 22, but a live `require_scope` grep shows 21 / 203 invocations). LLM-provider routes (holding API keys) require `llm:admin`; key-management endpoints require `keys:admin` as a privilege-escalation guard. Grant text below is taken from `scopes.py` (live read).
...
| `ip_allowlist` | `TEXT[]` | IP restriction — enforced at auth time; 403 on source-IP mismatch (Sprint 23b) |
| `is_service_account` | `BOOLEAN` | Service account flag |
| `created_at` | `TIMESTAMPTZ` | Creation timestamp |

#### 9.3.3 Enforcement

`require_scope(request, scope)` and `require_store_access(request, store_id)` are wired into **21 route files** (203 `require_scope` invocations, live grep — the Sprint-23b changelog rounded this to "22"). Every authenticated endpoint resolves to a defined scope. In no-auth mode (single-tenant), all scopes are granted. The `*` superuser scope bypasses all checks.
...
> [!TIP]
> The **full conformance suite passes 52/52 in mock mode**. Live runs differ per provider (artifact `GRAFOMEM-AUDIT-025 v2.1`): OpenAI gpt-4o-mini at the 39-test baseline; Anthropic/Gemini **36/46** (2026-06-22) — the 7 prod-500 contributors are fixed (v2.7.1, verified on the live deployment via a 52/52 mock run), so a confirming live re-run is expected ~43/46; residual misses are 2 env-blocked offline tamper tests and 1 replay defect (see §32). Expanded in Sprint 13 with Phase 22 covering health check endpoints. Every security mechanism is verified both that it blocks AND that it doesn't falsely block. All six production gates (§17.2) are closed.
...
|---|---|---|
| **Mock conformance** | 52/52 — governance, security, attestation, replay, HITL lifecycle, streaming, webhooks, PDF, SSO | ✅ Complete |
| **GMP self-conformance** | W2/W5/W6/W10 against `PostgresGMPBackend` (primary prod backend) — M8 = 1.000 (7/7) | ✅ Complete |
...
Phase 6 proposes migrating the **primary** vector backend from pgvector to Qdrant. As of v2.7 this is **not yet live**: pgvector remains primary in production and the Qdrant adapter (`qdrant_gmp.py`) is built but not wired as the default factory (R1–R8 unfulfilled). The Phase-6 decision is considered closed unless one of the following reopening triggers is observed:
...
> mode: **52/52** (also verified on the live Railway deployment, v2.7.1). Live per-provider (2026-06-22): OpenAI 39-test baseline · Anthropic/Gemini **36/46** — 7 prod-500 contributors since fixed; confirming re-run expected ~43/46 (see §32).
...
|---|---|---|---|---|
| Total tests (current suite) | 52 (22 phases) | 39 (17 phases) | 46 (live-executable) | 46 (live-executable) |
| Passed | **52** | **39** | **36** | **36** |
...
|---|---|---|---|
| **TRIP** | Trajectory-based Recognition of Identity Proof | Longitudinal proof-of-humanity / anti-Sybil attestation via spatially quantized breadcrumbs | Not implemented — TRIP is a physical-world identity protocol ([IETF draft-ayerbe-trip-protocol-04](https://datatracker.ietf.org/doc/draft-ayerbe-trip-protocol/04/)) |
...
| Hash chain | SHA-256 (epochs) | BLAKE2b-256 | SHA-256 |
...
> [!NOTE]
> Sprint 17 adds Anthropic (Claude Opus 4) and Gemini (2.5 Pro) as provider options alongside existing OpenAI and Mock. **What shipped**: adapter plumbing, CLI flags (`--anthropic` / `--gemini`), 16 DB-free unit tests, and live conformance runs for all three providers. OpenAI gpt-4o-mini runs at the 39-test baseline; live runs for Anthropic Claude Opus 4 and Google Gemini 2.5 Pro score **36/46** each (artifact `GRAFOMEM-AUDIT-025 v2.1`, 2026-06-22). The live denominator is 46, not 51: 5 offline DB-tamper tests are excluded because they require direct DB mutation, which is blocked against the remote prod network. Of the 10 live misses, **7 were production routing/500 errors on the Railway deployment, 2 are environment-blocked offline tamper tests, and 1 is a provider replay defect**. The 7 prod 500s were a connection-pool init-order bug, fixed in v2.7.1 (verified on the live deployment via a 52/52 mock-mode run; commit `3c2ab09`). A confirming live per-provider re-run is expected to clear those 7 (→ ~43/46); the residual 2 env-blocked + 1 replay are not product defects.
...
> - **Anthropic** (`--anthropic`): **36/46**, Claude Opus 4 (`claude-opus-4-20250514`)
> - **Gemini** (`--gemini`): **36/46**, Gemini 2.5 Pro
>
> Live denominator is 46 (5 offline DB-tamper tests excluded — they need direct DB mutation, blocked against the remote prod network). The 10 live misses were **7 production routing/500 errors**, **2 environment-blocked** offline tamper tests, and **1 provider replay defect**. The 7 prod 500s were a connection-pool init-order bug — **fixed in v2.7.1** (`create_app` pool init; commit `3c2ab09`) and verified on the live deployment (mock-mode `sandbox_e2e_v2.py` 52/52). A confirming live per-provider re-run is expected to land ~43/46; the remaining 2 env-blocked + 1 replay are not product defects.
...
| **Cloud managed inference** | LIVE | Multi-provider (§32): OpenAI `gpt-4o-mini` (39-test baseline), Anthropic/Gemini (**36/46**, 2026-06-22). The 7 prod routing/500 misses are fixed (v2.7.1, verified on the live deployment); confirming live re-run expected ~43/46. |
...
*End of document. Version 2.7.1 — 26 sprints. Mock conformance 52/52 + 50/50 v3 governed-services + 12/12 gcrumbs = 114 gates (mock/local); GMP self-conformance M8=1.000 (PostgresGMPBackend); HITL resume lifecycle validated two-sided; OpenAPI contract test validates SDK ↔ spec alignment; Prometheus observability with 18 metrics + health probes. Full `pytest tests/` suite 241/241 green (run together). Python SDK 32/33 against live sandbox. Live multi-provider conformance (2026-06-22): OpenAI 39-test baseline; Anthropic/Gemini 36/46 — the 7 prod-500 contributors were fixed in v2.7.1 (verified on the live deployment, mock 52/52, commit `3c2ab09`); a confirming live per-provider re-run is expected ~43/46. pgvector primary (Qdrant adapter built, Phase 6 not yet live); SIEM exporter parked; SLO alerting Defined-not-Active.*
